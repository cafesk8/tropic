    ...
    Shopsys.massEditConfirm.init = function ($container) {
        $container.filterAllNodes('.js-mass-edit-submit').click(function () {
            ...
            if (!isConfirmed) {
                ...
                var count;
                switch (selectType) {
                    case Shopsys.constant('\\App\\Model\\Product\\MassEdit\\MassEditFacade::SELECT_TYPE_CHECKED'):
                        count = $('.js-grid-mass-action-select-row:checked').length;
                        break;
                    case Shopsys.constant('\\App\\Model\\Product\\MassEdit\\MassEditFacade::SELECT_TYPE_ALL_RESULTS'):
