...
class Version20200810161233 extends AbstractMigration
{
    ...
    {
        $this->sql(
            'INSERT INTO order_statuses (type, check_order_ready_status, activates_gift_certificates)
            VALUES (5, false, false)'
        );
        $orderStatusId = (int)$this->connection->lastInsertId('order_statuses_id_seq');
        $this->createOrderStatusTranslationAndMailTemplate($orderStatusId, 'Nepřevzato zákazníkem');
        ...
        $orderStatusId = (int)$this->connection->lastInsertId('order_statuses_id_seq');
        $this->createOrderStatusTranslationAndMailTemplate($orderStatusId, 'Zaplaceno');
    }
